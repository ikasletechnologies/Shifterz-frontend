...
import {
  Receipt, Search, Filter, ChevronLeft, ChevronRight, Pencil, Trash2, Plus,
  Users, UserCheck2, UserX2, Briefcase, Loader2, CheckCircle2, TrendingUp, X
} from "lucide-react";
import { StatCard } from "@/components/dashboard/StatCard";
import EditEmployeeDialog from "@/components/employees/EditEmployeeDialog";
import AddEmployeeDialog from "@/components/employees/AddEmployeeDialog";
import { getBillingExecutiveManagementStats, getFranchises, createEmployee, updateEmployee, deleteEmployee } from "@/lib/api";
...
export default function BillingStaffPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const data = await getBillingExecutiveManagementStats(params);
      setRows(data.list || []);
      setSummary(data.summary || null);
      setTotal(data.total || 0);
    } catch (err: any) {
      toast.error("Failed to load billing staff: " + err.message);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleAdd = async (employeeData: any) => {
    try {
      ...
      toast.success("Billing executive created successfully");
      setIsAddOpen(false);
      setSearchTerm("");
      setBranchFilter("All");
      setStatusFilter("All");
      ...
      const data = await getBillingExecutiveManagementStats({ page: "1", pageSize: String(PAGE_SIZE) });
      setRows(data.list || []);
      setSummary(data.summary || null);
      setTotal(data.total || 0);
    } catch (err: any) {
      toast.error("Failed to create billing executive: " + err.message);
    }
  };
  ...
  const handleEdit = async (id: string, employee: any) => {
    try {
      ...
      toast.success("Billing executive updated");
      setIsEditOpen(false);
      fetchData();
    } catch (err: any) {
      toast.error("Failed to update billing executive: " + err.message);
    }
  };

  const handleDelete = async (id: string) => {
    if (!confirm("Remove this billing executive?")) return;
    try {
      await deleteEmployee(id);
      toast.success("Billing executive removed");
      fetchData();
    } catch (err: any) {
      toast.error("Failed to remove billing executive: " + err.message);
    }
  };
    ...
    <div className="p-4 sm:p-6 md:p-8 space-y-6">
      {summary && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <StatCard title="Total Billing Staff" value={summary.total} icon={Users} color="blue" onClick={() => { setStatusFilter("All"); setCurrentPage(1); }} active={statusFilter === "All"} />
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-gray-100">
          ...
          <p className="text-xs text-slate-500 mt-0.5">Billing executives raise invoices, record payments and close out paid jobs.</p>
        </div>
          ...
          <table className="data-table w-full text-sm text-left min-w-[800px]">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr><td colSpan={6} className="px-6 py-10 text-center text-gray-400">{searchTerm || statusFilter !== "All" || branchFilter !== "All" ? "No billing staff match these filters." : "No billing staff yet. Use “Add Billing Executive” to add the first one."}</td></tr>
              ) : (
                rows.map((row) => (
      ...
      {isEditOpen && editing && (
        <EditEmployeeDialog
          ...
          onEdit={handleEdit}
          employee={{ ...editing, role: editing.role || "BILLING_EXECUTIVE" }}
          franchises={franchises}
        />
      )}
    </div>
