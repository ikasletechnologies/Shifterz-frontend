...
function ReceptionistsPageContent() {
  ...
  const [editing, setEditing] = useState<ReceptionRow | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      toast.error("Failed to load receptionists: " + err.message);
    } finally {
      setIsLoading(false);
    }
  ...
  const openEdit = (row: ReceptionRow) => {
    setEditing(row);
    setIsEditOpen(true);
  };
  ...
  const handleEdit = async (id: string, employee: any) => {
    try {
      ...
      toast.error("Failed to update receptionist: " + err.message);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast.error("Failed to remove receptionist: " + err.message);
    }
  };
    ...
    <div className="p-4 sm:p-6 md:p-8 space-y-6">
      ...
      {summary && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <StatCard title="Total Receptionists" value={summary.total} icon={Users} color="blue" onClick={() => { setStatusFilter("All"); setCurrentPage(1); }} active={statusFilter === "All"} />
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-gray-100">
          ...
          <p className="text-xs text-slate-500 mt-0.5">Receptionists check vehicles in, register customers and hand out out-passes at the gate.</p>
        </div>
          ...
          <table className="data-table w-full text-sm text-left min-w-[800px]">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr><td colSpan={8} className="px-6 py-10 text-center text-gray-400">{searchTerm || statusFilter !== "All" || branchFilter !== "All" ? "No receptionists match these filters." : "No receptionists yet. Use “Add Receptionist” to add the first one."}</td></tr>
              ) : (
                rows.map((row) => (
      ...
      {isAddOpen && (
        <AddEmployeeDialog
          ...
          defaultRole="RECEPTION_EXECUTIVE"
        />
      )}
      ...
      {isEditOpen && editing && (
        <EditEmployeeDialog
          ...
          onEdit={handleEdit}
          employee={{ ...editing, role: editing.role || "RECEPTION_EXECUTIVE" }}
          franchises={franchises}
        />
      )}
    </div>
