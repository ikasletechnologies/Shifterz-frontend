...
export function HQDashboard({ allowedModules }: { allowedModules?: string[] | null }) {
  ...
  const showFranchises = !allowedModules || allowedModules.includes("employees") || allowedModules.includes("dashboard");
  const showRevenue = !allowedModules || allowedModules.includes("billing") || allowedModules.includes("payments");
  const showJobs = !allowedModules || allowedModules.includes("jobs") || allowedModules.includes("carin") || allowedModules.includes("outpass");
  const showInventory = !allowedModules || allowedModules.includes("inventory");

  const statCardsCount = [showFranchises, showRevenue, showJobs, showInventory].filter(Boolean).length;
