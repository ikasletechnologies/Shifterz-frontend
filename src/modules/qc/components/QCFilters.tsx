...
import { QC_FILTER_OPTIONS } from "../constants/qc.constants";
...
export function QCFilters({ statusFilter, searchQuery, onStatusChange, onSearchChange }: QCFiltersProps) {
  return (
    <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-between">
      <div
        className="rounded-lg px-2 py-1.5 flex items-center gap-1 flex-wrap"
        style={{ backgroundColor: "#ebebebff" }}
      >
        {QC_FILTER_OPTIONS.map((f) => (
          <button
            key={f}
